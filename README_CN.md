...
python -m pocket eval     # 126 条确定性评测，离线、免费，一秒内跑完
...
核心**只依赖标准库**，`pocket/*.py` 合计 **7,194 行**；`anthropic` / `openai` 只有在你指定
...
可复现的数字，不是形容词。离线那几项来自 `scripts/measure.py`；两组 A/B 走的是
`kimi-k2.5` 的普通 provider 路径，每组对照都用同一批轮次、同一个模型，只改一个变量。

| 机制 | 实测 |
|---|---|
| Skill catalog vs 正文全内联（10 个技能） | 常驻系统提示 **3,022 → 1,010 token，−66.6%** |
| — 每技能 | catalog 44 token vs 正文 195 token（**4.4×**） |
| 40KB 工具结果外置 | **40,000 → 763 字符，−98.1%** |
| 20 轮超预算会话压缩 | **14,472 → 3,135 字符，−78.3%**，折叠 13 条消息 |
| 上下文治理开 vs 关，同一任务 | 输入 token **13,370 → 2,118，−84.2%**（总计 −82.6%） |
| 分诊把一轮路由出循环 | **5/8 轮**；约 2 次模型调用而非约 4.3 次，**−53%**，且提示里不带工具表 |
| 检索门控，12 条标注用例 | **召回率 100%（零漏检）**，代价加权准确率 **0.90** |
| — 无关记忆注入 | 该跳过的用例里 **100% → 50%** |

其中两项要说清它们不是什么。分诊那行数的是**模型调用次数而不是延迟**，因为测它用的免费档
有速率限制，那里的延迟测的是限速器。检索门控的**普通准确率是 75%**——加权后更高，是因为
它那三次错误全部是"多查了一次"而不是"漏了一次"，而给这两种错误定不同的价正是这个指标存在
的理由。
